...
class FungoVisual:
    ...
    def plotMatrix(playerName: str, plotType: str, pitchMatrix: np.array) -> None:
        ...
        if plotType == "BA":
            plt.title("Batting Average by zone", fontsize=20)
        elif plotType == "Whiff":
    ...
    def plotRollingAvg(playerName: str, playerData: list[float]) -> None:
        ...
        """
        fig, ax = plt.subplots()
        ax.plot(range(1, len(playerData) + 1), playerData)
